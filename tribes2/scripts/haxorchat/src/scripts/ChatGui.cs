...
//	if(ChatMessageEntry.textCheck) // haxored by teh powdddddddddaaaaaa! w00t
//	{
   ...
   if (%p && %c)
      if (%c.removeMember(%p))
     {
         ...
         if (%p == $IRCClient::people.getObject(0))
         {
            ...
            IRCClient::statusMessage(%host @ " smacks " @ IRCClient::taggedNick(%p) @ " out of here!");
            IRCClient::notify(IDIRC_KICK);
         }
         else
         {
            IRCClient::newMessage(%c, %host @ " smacks " @ IRCClient::taggedNick(%p) @ " out of this room!");
         IRCClient::notify(IDIRC_PART);
       }
...
function IRCClient::onJoinGame(%address, %desc)
{
   ...
   %joinLink = "<t2server:" @ %address @ ">click here to follow my sexy ass!</t2server>.";
